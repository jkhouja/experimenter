...
class RNNModel(experimenter.modeling.BaseModel):
    ...
    def __init__(self, config):
        ...
        self.lstm = torch.nn.LSTM(input_size=self.embedding_dim, hidden_size=self.hidden_dim, num_layers= self.num_layers, dropout=self.dropout, batch_first=True)
    ...
    def forward(self, input_batch, **kwargs):
        ...
        output = [self.linear(self.batch_norm(s1_last_hidden[0][self.num_layers - 1, :, :].squeeze()).squeeze())]
