...
class MultiLMPairProvider(DataProvider):
    ...
    def upload_data(
        ...
        for split in splits:
            ...
            data = [
                {"inp": [d2[:-2], d[:-2]], "label": [d2[1:], [l]], "mask": [1, 0]}
                for d, d2, l in zip(s1_data, s2_data, labels)
            ]
            ...
            data.extend(
                [
                    {"inp": [d[:-2], d2[:-2]], "label": [d[1:], [l]], "mask": [1, 150]}
                    for d, d2, l in zip(s1_data, s2_data, labels)
                ]
            )
            # data.extend([{'inp':[d2[:-2], d2[:-2]], 'label':[d2[1:],\
            # ["agree"]], 'mask':[1,70]} for d, d2, l in zip(s1_data, s2_data, labels)])
            out.append(data)
