...
class RNNMultiLMPairModel(BaseModel):
    ...
    def __init__(self, config):
        ...
        self.cls_linear = torch.nn.Linear(
            1, self.num_classes
        )  # This is binary classification, we will output 1
        self.lm_linear = torch.nn.Linear(
            self.hidden_dim, self.lm_classes
        )  # This is binary classification, we will output 1
    ...
    def forward(self, input_batch, **kwargs):
        ...
        s1_all_hidden = torch.nn.utils.rnn.pad_packed_sequence(
            s1_all_states, batch_first=True, padding_value=0, total_length=self.seq_len
        )

        s2_lengths = (
            (s2 > 0).type(torch.DoubleTensor).sum(dim=1)
        )  # TODO: Move length calculations to preprocessing
        s2_packed = torch.nn.utils.rnn.pack_padded_sequence(
            s2_emb, s2_lengths, batch_first=True, enforce_sorted=False
        )
        s2_all_states, s2_last_hidden = self.lstm(s2_packed, first_hidden)
        # s2_all_hidden = torch.nn.utils.rnn.pad_packed_sequence(s2_all_states,\
        # batch_first=True, padding_value=0, total_length=self.seq_len)
        # last_hidden = torch.nn.utils.rnn.pad_packed_sequence(last_hidden[0], \
        # batch_first=True, padding_value=0, total_length=self.seq_len)
        # Squeeze to remove first dimension (num_layers/directions)
        # perspective = s1_last_hidden[0].squeeze(). * s2_last_hidden[0].squeeze()
        perspective = self.similarity(
            s1_last_hidden[0].squeeze(), s2_last_hidden[0].squeeze()
        )
