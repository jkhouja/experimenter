...
class PairClsProvider(DataProvider):
    ...
    def __init__(self, config):
        ...
        self.encoder = {}
        self.encoder["inp"] = [
            U.chainer(funcs=[cleaner, tokenizer, enc]),
            U.chainer(funcs=[cleaner, tokenizer, enc]),
        ]
    ...
    def upload_data(
        ...
        Args:
            input_path: In config file, should be csv and contains 3 columns: s1, s2, and stance
        """
        ...
        for split in splits:
            ...
            data = [
                {"inp": [d, d2], "label": [[label]], "mask": [1]}
                for d, d2, label in zip(s1_data, s2_data, labels)
            ]
