...
class PairClsProvider(DataProvider):
    ...
    def __init__(self, config):
        ...
        self.label_col_name = self.args['label']

        self.encoder = {}
        self.encoder['inp'] = [U.chainer(funcs=[cleaner, tokenizer, enc]), U.chainer(funcs=[cleaner, tokenizer, enc])]
        ...
        self.decoder = {}
        self.decoder['inp'] = [U.chainer(funcs=[enc.decode, tokenizer.detokenize]), U.chainer(funcs=[enc.decode, tokenizer.detokenize])]
        ...
        self.logger.info(f"Vocab Length: {len(enc.vocab)}")
    ...
    def upload_data(self,  **kwargs) -> List[Tuple[List[Union[List[int],int]], List[Union[List[int],int]], List[int]]]:
        ...
        for split in splits:
            #data_in = split
            s1_data = [x[self.input_a] for x in split]
            s2_data = [x[self.input_b] for x in split]
            labels = [x[self.label_col_name] for x in split]

            data = [{'inp':[d, d2], 'label':[[label]], 'mask':[1]} for d, d2, label in zip(s1_data, s2_data, labels)]
