...
class RNNLModel(torch.nn.Module):
    # Basic LSTM model
    def __init__(self, config):
        super(RNNLModel, self).__init__()
    ...
    def forward(self, input_batch, **kwargs):
        ...
        out_layer = self.linear(all_hidden[0])
...
class RNNPairModel(torch.nn.Module):
    ...
    def forward(self, input_batch, **kwargs):
        ...
        prediction = self.linear(perspective).squeeze()
        ...
        res = []
        try:
            res.extend([s.argmax(dim=1).tolist() for s in output])
        except IndexError as e:
            # batch_size = 1 or last batch
            res.extend([[s.argmax().tolist() for s in output]])
        return output, (res, input_batch)
