...
class DictDataProvider():
    ...
    def _from_batch_old(self, minibatch: dict):
        ...
        return res
    def _from_batch(self, minibatch: dict):
        """Regenerate dict of data by stiching batches accross dictionary keys.  Implementation might not be correct"""
        ...
        for key in raw_data.keys():
            ...
            for i, feat in enumerate(raw_data[key]):
                try:
                    ...
                    self.logger.error(f"Failed encoding feature {i} in key {key}. Will be passed as is")
                    raise
                    res[key].append(feat)
            ...
            for key in self.keys:
                ...
                for j, feat in enumerate(self.data[key]):
                    ...
                    if isinstance(feat[idx], list):
                        tmp = np.zeros((self.lims[key][j]), dtype=int)
                        tmp[:min(self.lims[key][j], len(feat[idx]))] = feat[idx][:min(self.lims[key][j], len(feat[idx]))]
                        returned[key].append(tmp)
                    else:
