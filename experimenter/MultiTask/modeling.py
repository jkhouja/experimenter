...
class SharedEncoder(BaseModel):
    """A model that takes a text sequence and a list of (classes, sequences), creates a representation and output a decoder for sequence and/or label per class"""
    def __init__(self, config):
        ...
        for i in range(self.num_outputs):
                self.out_decoder.append(torch.nn.Linear(self.hidden_dim, self.num_classes[i]))
    ...
    def forward(self, input_batch, **kwargs):
        ...
        inp_text_lengths = (inp_text > 0).type(torch.DoubleTensor).sum(dim=1) #TODO: Move length calculations to preprocessing
        s1_packed = torch.nn.utils.rnn.pack_padded_sequence(inp_text_emb, inp_text_lengths, batch_first=True, enforce_sorted=False)
        s1_all_states, s1_last_hidden = self.lstm(s1_packed, first_hidden)
        # s1_last_hidden = (h_n, c_n) each of shape (num_layers * num_directions, batch, hidden_size):
        s1_last_state = s1_last_hidden[0].permute(1,2,0).squeeze()
        s1_all_hidden = torch.nn.utils.rnn.pad_packed_sequence(s1_all_states, batch_first=True, padding_value=0, total_length=self.in_seq_len[0])
        ...
        try:
            ...
            res.extend([s.argmax(dim=1, keepdim=False) if s.dim() > 2 else s.argmax(dim=1, keepdim=True) for s in output])
...
class SeqDecoder(BaseModel):
    def __init__(self, config, emb_layer, inp_dim, hidden_dim, num_classes, max_seq_len):
        ...
        self.lstm = torch.nn.LSTM(input_size=self.inp_dim, hidden_size=self.hidden_dim, dropout=self.dropout, batch_first=True)
    ...
    def forward(self, input_batch, **kwargs):
        ...
        inp_text_lengths = (input_batch > 0).type(torch.DoubleTensor).sum(dim=1) #TODO: Move length calculations to preprocessing
        s1_packed = torch.nn.utils.rnn.pack_padded_sequence(inp_text_emb, inp_text_lengths, batch_first=True, enforce_sorted=False)
        s1_all_states, s1_last_hidden = self.lstm(s1_packed, first_hidden)
        # s1_last_hidden = (h_n, c_n) each of shape (num_layers * num_directions, batch, hidden_size):
        s1_last_state = s1_last_hidden[0].permute(1,2,0).squeeze()
        s1_all_hidden = torch.nn.utils.rnn.pad_packed_sequence(s1_all_states, batch_first=True, padding_value=0, total_length=self.max_seq_len)
        # s1_all_hidden shape is (seq_len, batch, num_directions * hidden_size):
        # representation is s1_last_hidden
...
class Seq2Seq(BaseModel):
    """An encoder decoder model that takes a text sequence and a list of (classes, sequences), creates a representation and output a decoder for sequence and/or label per class"""
    def __init__(self, config):
        ...
        for i in range(self.num_outputs):
            if self.encoders[i] == "text":
                ...
                self.out_decoder.append(SeqDecoder(self.config, self.emb, self.embedding_dim, self.hidden_dim, self.num_classes[i], self.out_seq_len[i]))
            else:
                self.out_decoder.append(torch.nn.Linear(self.hidden_dim, self.num_classes[i]))
